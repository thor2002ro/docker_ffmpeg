...
RUN     apt-get -yqq update && \
        apt-get install -yq --no-install-recommends ca-certificates expat libgomp1 software-properties-common gpg-agent && \
        add-apt-repository ppa:kisak/kisak-mesa -y && \
        apt-get -yqq update && apt-get dist-upgrade -y && \
        apt-get install -yq --no-install-recommends ca-certificates expat libgomp1 libdrm-amdgpu1 libdrm-common libdrm-intel1 libdrm-nouveau2 libdrm-radeon1 libdrm2 libegl-mesa0 libgbm1 libgl1-mesa-dri libglapi-mesa libglx-mesa0 libxatracker2 mesa-vulkan-drivers && \
...
RUN      buildDeps="autoconf \
                    ...
                    perl \
                    pkg-config \
                    python-is-python3 python3-xcbgen \
                    libssl-dev \
                    yasm \
                    libva-dev \
                    zlib1g-dev" && \
...
RUN \
        ...
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        git clone https://code.videolan.org/videolan/x264.git ${DIR} --depth 1 && \
...
RUN \
        ...
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        git clone https://bitbucket.org/multicoreware/x265_git.git ${DIR} --depth 1 && \
        cd ${DIR}/build/linux && \
        rm -r ${DIR}/.git && \
...
RUN \
        ...
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        git clone https://github.com/zeromq/libzmq.git ${DIR} --depth 1 && \
        ./autogen.sh && \
        ./configure --prefix="${PREFIX}" && \
        make && \
        make install && \
...
RUN \
        ...
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        git clone https://github.com/Haivision/srt.git ${DIR} --depth 1 && \
        cmake -DCMAKE_INSTALL_PREFIX="${PREFIX}" . && \
        make && \
        make install && \
        rm -rf ${DIR}
...
LABEL       org.opencontainers.image.authors="thor2002ro@gmail.com" \
...
RUN     apt-get update -yqq && apt-get -y install software-properties-common && \
        ...
        apt-get -yqq update && apt-get dist-upgrade -y && \
	apt-get update -y && \
	apt-get install -y --no-install-recommends libva-drm2 libva2 i965-va-driver mesa-va-drivers vainfo libdrm-amdgpu1 libdrm-common libdrm-intel1 libdrm-nouveau2 libdrm-radeon1 libdrm2 libegl-mesa0 libgbm1 libgl1-mesa-dri libglapi-mesa libglx-mesa0 libxatracker2 mesa-vulkan-drivers && \
